...
# Load the Excel file
file_path = 'C:/Users/user/OneDrive/Documents/Advanced Willy/NUFDI/NUFDI/WSJ.xlsx'  # Replace with your actual file path
...
replacements = {'protest': 'protest', 'protester': 'protest', "protesters": 'protest', 'protests': 'protest', 
                'sanction': 'sanction', 'sanctions': 'sanction', 'economy': 'sanction', 'economic': 'sanction', 'reform':'reform', 'reformist': 'reform', 'reformists':'reform', 'regime': 'regime', 'islamic': 'regime', 'republic':'regime'}
...
wordcloud_obj.to_file('wordcloud_high_res_WSJ.png')
...
specific_words = ['sanction', 'demand', 'democracy', 'reform', 'government', 'regime', 'antigovernment', 'antiregime', 'nuclear', 'sanction', 'tehran', 'kurdish', "israel"]
...
plt.savefig('top_10_words1_WSJ.png')  # Save the figure as an image file
plt.show()
...
plt.tight_layout()
plt.savefig('cooccurrence_counts1WSJ.png')
plt.show()
...
plt.savefig('sentiment_distributionWSJ.png')
plt.show()
...
filtered_df.to_csv(f'filtered_sentences_{specific_word}_WSJ.csv', index=False)
...
plt.savefig(f'sentiment_distribution_{specific_word}_WSJ.png')
plt.show()
...
for pair in words_to_compare:
    ...
    plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=['pink', 'lightgreen'])
    plt.title(f'Comparison of {pair[0]} and {pair[1]}', bbox={'facecolor': 'w', 'edgecolor': 'w', 'pad': 10})
    plt.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    plt.tight_layout()
    plt.savefig(f'{pair[0]}_vs_{pair[1]}_WSJ.png')  # Save each pie chart as an image file
    plt.show()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('sentiment_over_time_violin_WSJ.png')
plt.show()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('state_media_mentions_ratio_over_time_WSJ.png')
plt.show()
