...
# Load the Excel file
file_path = 'C:/Users/user/OneDrive/Documents/Advanced Willy/NUFDI/NUFDI/AP.xlsx'  # Replace with your actual file path
...
wordcloud_obj.to_file('wordcloud_high_res_AP.png')
...
specific_words = ['sanction', 'demand', 'democracy', 'reform', 'government', 'regime', 'antigovernment', 'antiregime', 'nuclear', 'sanction', 'tehran', 'kurdish']
...
plt.savefig('top_10_wordsAP.png')  # Save the figure as an image file
plt.show()
...
plt.tight_layout()
plt.savefig('cooccurrence_counts_AP.png')
plt.show()
...
filtered_df.to_csv(f'filtered_sentences_{specific_word}_AP.csv', index=False)
...
plt.savefig(f'sentiment_distribution_{specific_word}_AP.png')
plt.show()

# Define the words to compare
words_to_compare = [
    (specific_words[0], specific_words[1]),  # sanction vs. demand
    (specific_words[2], specific_words[3]),  # democracy vs. reform
    (specific_words[4], specific_words[5]),  # government vs. regime
    (specific_words[6], specific_words[7]),  # antigovernment vs. antiregime
    (specific_words[8], specific_words[9]),  # nuclear vs. sanction
    (specific_words[10], specific_words[11])  # tehran vs. kurdish
]
...
for pair in words_to_compare:
    ...
    plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=['purple', 'blue'])
    plt.title(f'Comparison of {pair[0]} and {pair[1]}', bbox={'facecolor': 'w', 'edgecolor': 'w', 'pad': 10})
    plt.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    plt.tight_layout()
    plt.savefig(f'{pair[0]}_vs_{pair[1]}_AP.png')  # Save each pie chart as an image file
    plt.show()
...
plt.figure(figsize=(12, 6), dpi=300)
sns.violinplot(x=df['Data'].dt.to_period('M').astype(str), y=df['sentiment'])
plt.xlabel('Date')
plt.ylabel('Sentiment Score')
plt.title('Sentiment Score Over Time')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('sentiment_over_time_violin_AP.png')
plt.show()
...
media_mentions_over_time['Data'] = media_mentions_over_time['YearMonth'].dt.to_timestamp()

# Plot the ratio of state media mentions to the number of articles published each month
plt.figure(figsize=(12, 6), dpi=300)
sns.lineplot(x='Data', y='mention_ratio', data=media_mentions_over_time, marker='o')
plt.xlabel('Date')
plt.ylabel('Mentions to Articles Ratio')
plt.title('Ratio of State-Owned Media Mentions to Articles Published Over Time')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('state_media_mentions_ratio_over_time_AP.png')
plt.show()

# Plot the number of articles published each month
plt.figure(figsize=(12, 6), dpi=300)
sns.lineplot(x='Data', y='article_count', data=article_counts, marker='o')
plt.xlabel('Date')
plt.ylabel('Number of Articles')
plt.title('Number of Articles Published Each Month')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('articles_per_month_AP.png')
plt.show()
