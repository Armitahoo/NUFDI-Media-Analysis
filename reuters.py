...
# Load the Excel file
file_path = 'C:/Users/user/OneDrive/Documents/Advanced Willy/NUFDI/NUFDI/The Hill.xlsx'
...
wordcloud_obj.to_file('wordcloud_high_res1_hill.png')
...
plt.savefig('top_10_words1_hill.png')  # Save the figure as an image file
plt.show()
...
plt.tight_layout()
plt.savefig('cooccurrence_counts1_hill.png')
plt.show()
...
plt.savefig(f'sentiment_distribution_{specific_word}.png')
plt.show()
...
for pair in words_to_compare:
    ...
    plt.tight_layout()
    plt.savefig(f'{pair[0]}_vs_{pair[1]}_hill.png')  # Save each pie chart as an image file
    plt.show()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('sentiment_over_time_violin_hill.png')
plt.show()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('state_media_mentions_ratio_over_time_hill.png')
plt.show()
