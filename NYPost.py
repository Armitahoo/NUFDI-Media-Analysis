...
# Load the Excel file
file_path = 'C:/Users/user/OneDrive/Documents/Advanced Willy/NUFDI/NUFDI/NYPost.xlsx'
...
wordcloud_obj.to_file('wordcloud_high_res1_NYP.png')
...
specific_words = ['sanction', 'right', 'demand', 'reform', 'government', 'regime', 'antigovernment', 'antiregime', 'nuclear', 'sanction', 'tehran', 'kurdish']
...
plt.savefig('top_10_words1_Post.png')  # Save the figure as an image file
plt.show()
...
plt.tight_layout()
plt.savefig('cooccurrence_counts1_Post.png')
plt.show()
...
plt.savefig('sentiment_distribution_Post.png')
plt.show()
...
plt.savefig(f'sentiment_distribution_{specific_word}_NYPost.png')
plt.show()
...
for pair in words_to_compare:
    ...
    plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=['orange', 'darkred'])
    plt.title(f'Comparison of {pair[0]} and {pair[1]}', bbox={'facecolor': 'w', 'edgecolor': 'w', 'pad': 10})
    plt.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    plt.tight_layout()
    plt.savefig(f'{pair[0]}_vs_{pair[1]}_NYPost.png')  # Save each pie chart as an image file
    plt.show()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('sentiment_over_time_violin_Post.png')
plt.show()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('state_media_mentions_ratio_over_time_NYPost.png')
plt.show()
