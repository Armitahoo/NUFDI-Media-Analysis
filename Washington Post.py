...
# Load the Excel file
file_path = 'C:/Users/user/OneDrive/Documents/Advanced Willy/NUFDI/NUFDI/Washington Post.xlsx'  # Replace with your actual file path
df = pd.read_excel(file_path)

# Create two separate DataFrames based on whether 'Text' starts with 'Opinion'
df_opinion = df[df['Text'].str.startswith('Opinion', na=False)]
df_non_opinion = df[~df['Text'].str.startswith('Opinion', na=False)]
...
replacements = {'protest': 'protest', 'protester': 'protest', "protesters": 'protest', 'protests': 'protest', 
                'sanction': 'sanction', 'sanctions': 'sanction', 'economy': 'sanction', 'economic': 'sanction', 'reform':'reform', 'reformist': 'reform', 'reformists':'reform', "kurdish": "kurdish", "kurd": "kurdish", "kurds": "kurdish"}
...
# Function to process a DataFrame and create word cloud and plots
def process_text_data(df, output_prefix):
    ...
    wordcloud_obj.to_file(f'{output_prefix}_wordcloud_high_res.png')
    ...
    specific_words = ['sanction', 'demand', 'antiregime', 'reform', 'government', 'regime', 'antigovernment', 'antiregime', 'activism', 'sanction', 'regional', 'kurdish']
    ...
    plt.savefig(f'{output_prefix}_top_10_words.png')  # Save the figure as an image file
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'{output_prefix}_cooccurrence_counts.png')
    plt.show()

    # Define the words to compare
    words_to_compare = [
        (specific_words[0], specific_words[1]),  # sanction vs. demand
        (specific_words[2], specific_words[3]),  # antiregime vs. reform
        (specific_words[4], specific_words[5]),  # government vs. regime
        (specific_words[6], specific_words[7]),  # antigovernment vs. antiregime
        (specific_words[8], specific_words[9]),  # activism vs. sanction
        (specific_words[10], specific_words[11])  # regional vs. kurdish
    ]
    ...
    for pair in words_to_compare:
        ...
        plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=['darkblue', 'darkgreen'])
        plt.title(f'Comparison of {pair[0]} and {pair[1]}', bbox={'facecolor': 'w', 'edgecolor': 'w', 'pad': 10})
        plt.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
        plt.tight_layout()
        plt.savefig(f'{output_prefix}_{pair[0]}_vs_{pair[1]}.png')  # Save each pie chart as an image file
        plt.show()
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f'{output_prefix}_state_media_mentions_ratio_over_time.png')
    plt.show()
    ...
    plt.savefig(f'{output_prefix}_sentiment_distribution.png')
    plt.show()

    # Sentiment analysis for specific word
    specific_word = 'sanction'
    ...
    filtered_df.to_csv(f'{output_prefix}_filtered_sentences_{specific_word}.csv', index=False)
    ...
    plt.savefig(f'{output_prefix}_sentiment_distribution_{specific_word}.png')
    plt.show()
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f'{output_prefix}_sentiment_over_time_violin.png')
    plt.show()

    # Create a new column to check if both 'kurdish' and 'regional' appear in the text
    df['kurdish_and_regional'] = df['Text'].apply(lambda x: bool(re.search(r'\bregional\b', x, re.IGNORECASE)) and bool(re.search(r'\bstate\b', x, re.IGNORECASE)))

    # Aggregate the counts by month
    df['YearMonth'] = df['Date'].dt.to_period('M')
    cooccurrence_over_time = df[df['kurdish_and_regional']].groupby('YearMonth').size().reset_index(name='count')

    # Convert 'YearMonth' to timestamp for plotting
    cooccurrence_over_time['Date'] = cooccurrence_over_time['YearMonth'].dt.to_timestamp()

    # Plot the results
    plt.figure(figsize=(12, 6), dpi=300)
    sns.lineplot(x='Date', y='count', data=cooccurrence_over_time, marker='o', color='orange')
    plt.xlabel('Date')
    plt.ylabel('Number of Articles')
    plt.title('Frequency of Articles Mentioning Both "Kurdish" and "Regional" Over Time')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f'{output_prefix}_kurdish_regional_frequency_over_time.png')
    plt.show()

# Process opinion and non-opinion texts separately
process_text_data(df_opinion, 'opinion')
process_text_data(df_non_opinion, 'non_opinion')
